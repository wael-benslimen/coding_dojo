              ...
              <tbody>
                  {allBooks.map(b => (
                      <tr key={b._id}  className=' border-b' >
                          ...
                          <td className=' px-6 py-3'> {b.isAvailable ? 'Available' : 'Unavailable'} | <Link to={`/edit/${b._id}`} className=' text-blue-500 underline font-semibold'>edit</Link> </td>
                          <td className=' px-6 py-3'><Link to={`/books/${b._id}`} className=' bg-blue-500 p-1 rounded-lg border text-white border-black text-l' onClick={()=>pageSetter('Book details')}>view details</Link></td>
                      </tr>
                  ))}
                  </tbody>
              </table>
